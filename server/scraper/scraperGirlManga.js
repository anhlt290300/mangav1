async function autoScroll(page) {
  ...
  const scraper_GirlManga = (browser, url) =>
    new Promise(async (res, reject) => {
      try {
        ...
        for (let i = 0; i < 30; i++) {
          await newPage.click(
            ".Module.Module-211 > .ModuleContent > .items-slide > a.prev",
            { delay: 30 }
          );
        }
        ...
        const topmangas = await newPage.$$eval(
          ".Module.Module-211 > .ModuleContent > .items-slide >  .owl-carousel.clearfix.owl-theme > .owl-wrapper-outer > .owl-wrapper > .owl-item",
          (els) => {
        ...
        const girlmangas = await newPage.$$eval(
          ".Module.Module-198 > .ModuleContent > .items > .row > .item",
          (els) => {
            girlmangas = els.map((el) => {
              const page = "https://www.nettruyenvt.com";
  ...
  module.exports = scraper_GirlManga;
